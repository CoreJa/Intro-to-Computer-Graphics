...
void main(){
    ...
    gl_Position = vec4(a_position.x*scale,
                       a_position.y*scale,
                       1,1);
...
function initWebGL(){
  ...
  */
  ...
  let scale=1;
  draw(gl,program,buffer,scale);
  const range=document.querySelector("#range");
  range.addEventListener("input",()=>{
    scale=parseFloat(range.value);
    draw(gl,program,buffer,scale);
    // console.log(scale);
  })
}
...
function draw(gl,program,buffer,scale) {
  ...
  let scaleAttributeLocation=gl.getUniformLocation(program,'scale');
  //send in the data of scale
  gl.uniform1f(scaleAttributeLocation, scale);
